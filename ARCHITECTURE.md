...
| **Views** | Handles business logic, returns responses | `core/views.py` (6 view classes) |
| **Forms** | Validates user input, creates/updates models | `core/forms.py` (4 form classes) |
...
```mermaid
graph LR
    subgraph "Project Root: Helping Hand MAX"
        ...
        subgraph "core/ — Main Application"
            H["models.py<br/>4 data models (525 lines)"]
            I["views.py<br/>6 view classes (343 lines)"]
            J["forms.py<br/>4 form classes (250 lines)"]
            K["urls.py<br/>6 URL patterns"]
            L["admin.py<br/>4 admin configs (163 lines)"]
            M["managers.py<br/>Custom QuerySets (110 lines)"]
            N["pricing.py<br/>Pricing engine (133 lines)"]
...
| File | Purpose | Size |
|------|---------|------|
| `core/models.py` | User, StaffProfile, Booking, Review models | 525 lines |
| `core/views.py` | Landing, Services, Support, Auth views | 343 lines |
| `core/forms.py` | Registration, Login, Profile forms | 250 lines |
| `core/admin.py` | Admin panel customization | 163 lines |
| `core/pricing.py` | Service rates & discount calculations | 133 lines |
| `core/managers.py` | Booking & StaffProfile custom querysets | 110 lines |
| `core/decorators.py` | `@patient_required`, `@agency_required`, `@profile_complete_required` | 70 lines |
| `core/mixins.py` | `PatientRequiredMixin`, `AgencyRequiredMixin`, `ProfileCompleteRequiredMixin` | 71 lines |
| `core/validators.py` | `phone_validator` (10 digits), `pincode_validator` (6 digits) | 21 lines |
| `helpinghand/settings.py` | Django configuration (DB, Auth, etc.) | 122 lines |
| `templates/base.html` | Master layout with Tailwind + HTMX + Lucide | 114 lines |
...
### User Model (`core/models.py:35–115`)
...
> **Important:** `AUTH_USER_MODEL = 'core.User'` in `settings.py:90` replaces Django's default User model.

### StaffProfile Model (`core/models.py:121–219`)
...
### Booking Model — Business Rules (`core/models.py:225–480`)
...
### Review Model (`core/models.py:486–525`)
...
sequenceDiagram
    ...
    else POST Request (e.g., Registration)
...
| URL Pattern | View Class | Template | Description |
|-------------|-----------|----------|-------------|
| `/` | `LandingView` | `core/landing.html` | Hero, services, how-it-works, pricing, testimonials |
| `/services/` | `ServicesView` | `core/services.html` | Detailed service cards with features |
| `/support/` | `SupportView` | `core/support.html` | Searchable FAQ accordion + contact |
| `/auth/login/` | `CustomLoginView` | `auth/login.html` | Username + password login |
| `/auth/register/` | `RegisterView` | `auth/register.html` | Role-based registration |
| `/auth/logout/` | `CustomLogoutView` | — | Logout + redirect to landing |
| `/admin/` | Django Admin | Built-in | Full admin panel |
...
### Registration Code (`core/views.py:266–308`)
...
### Auth Settings (`settings.py:100–103`)
...
### Status Values (`core/models.py:239–246`)
...
### Automatic Status Determination (`core/models.py:426–434`)
...
### Booking ID Generation (`core/models.py:406–409`)
...
### Staff Matching Logic (`core/managers.py:81–96`)
...
graph TD
    subgraph "Template Hierarchy"
        A["base.html<br/>Master Layout<br/>(114 lines)"]
        A --> B["core/landing.html<br/>(15,823 bytes)"]
        A --> C["core/services.html<br/>(5,875 bytes)"]
        A --> D["core/support.html<br/>(7,474 bytes)"]
        A --> E["auth/login.html<br/>(3,566 bytes)"]
        A --> F["auth/register.html<br/>(6,447 bytes)"]
    end

    subgraph "Partials ({% include %})"
        G["core/_navbar.html<br/>(5,979 bytes)"]
        H["core/_footer.html<br/>(6,021 bytes)"]
    end
...
graph TD
    subgraph "Test Suite (core/tests/)"
        A["test_models.py<br/>(19,714 bytes)"]
        B["test_auth.py<br/>(14,206 bytes)"]
    end
...
flowchart TB
    subgraph "Patient Journey"
        ...
        P4 --> P5["5. Create Booking<br/>(service type, dates, patient info)"]
...
*Architecture documentation for the Helping Hand MAX codebase — a Django healthcare marketplace migrated from React/Firebase.*
*Generated: May 2026*
